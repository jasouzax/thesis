...
def main():
    if len(sys.argv) < 2:
        print("Usage: python flatcalibrate.py <baseline_cm>")
        sys.exit(1)
    baseline_arg = float(sys.argv[1])

    # Load flattening parameters
    flat_mtx, flat_dist = load_flatten_params()
    print(f"Loaded {FLATTEN_FILE}: Mtx shape {flat_mtx.shape}, Dist shape {flat_dist.shape}")

    print("--- Flat Auto-Capture Stereo Calibration ---")
    print("CONTROLS:")
    ...
    while True:
        ...
        cv2.imshow('Stereo Calib (FLAT)', combined)
    ...
    print("Calibrating (Stereo)... using FIXED intrinsics from flatten.json")
    img_size = (w, h)
    
    # We already have intrinsics from flatten.json
    # Since we UNDISTORTED the images, the effective distortion is 0
    # and the effective camera matrix is flat_mtx
    mtx_l = flat_mtx
    mtx_r = flat_mtx
    dist_l = np.zeros_like(flat_dist) # ZERO distortion
    dist_r = np.zeros_like(flat_dist) # ZERO distortion
    ...
    # Note: d1/d2 should be 0 here, M1/M2 should be flat_mtx (approx)
    ...
    data = { 
        ...
        "fov_left": fov_x_l,
        "fov_right": fov_x_r,
        "is_flat_calibration": True
    }
    
    fname = f"flattenstereo-{baseline_arg}.json"
    ...
    # Save stereo.json with HEAD pointing to current
    with open("stereo.json", 'w') as f: 
        json.dump({
            ...
            "pixel_error": ret_s,
            "calculated_baseline": calc_baseline,
            "user_baseline_arg": baseline_arg,
            "history": history
        }, f, cls=NumpyEncoder, indent=4)
        
    print(f"\nCalibration saved to {fname}")
    print(f"Summary updated in stereo.json")
